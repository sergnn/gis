...
class Map:
    ...
    def get_tile(self, x: int, y: int, z: int) -> QPixmap:
        ...
        local_tile = self.local_tile(x, y, z)

        if not local_tile.exists():
            download(self.get_tile_url(x, y, z), local_tile)

        loaded_tile = QPixmap(str(local_tile))
...
class YandexSatellite(Map):
    ...
    def get_tile_url(self, x, y, z):
        return f'https://sat0{randint(1, 4)}.maps.yandex.net/tiles?l=sat&scale=1&lang=ru_RU&x={x}&y={y}&z={z - 1}'
